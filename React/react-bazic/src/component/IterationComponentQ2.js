...
function IterationComponentQ2(){
  ...
  let val = lidata.map( (v, i) =>  
  <li key={i} >{v.type}-{v.teacher}</li> 
  )
  ...
  let handleSelect = (e) => {
    let selectlis = lidata.filter((v) => (v.type) == e.target.value).map( (v, i) =>  
                        <li key={i} >{v.type}-{v.teacher}</li> );
  ...
  let handleClick = () => {
    ...
    let selectlis = lidata.filter((v) => (v.type) == tagName.current.value || (v.teacher) == tagName.current.value).map( (v, i) =>  
                        <li key={i} >{v.type}-{v.teacher}</li> );
  ...
  return(
    <div>
      <h3>실습</h3>
      <input type="text" ref={tagName} /><button onClick={handleClick}>찾기</button><br />
